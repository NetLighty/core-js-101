...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (rect2.top < rect1.top + rect1.height) return true;
  return false;
}
...
function isCreditCardNumber(ccn) {
  const splitedCcn = String(ccn).split('').map((el) => Number(el));
  const digitsSum = splitedCcn.map(
    (el, index) => (index % 2 === splitedCcn.length % 2 ? el * 2 : el),
  ).map((el) => (el > 9 ? el - 9 : el)).reduce((a, b) => a + b, 0);
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (brackets[str[i]]) {
      buffer.push(str[i]);
    } else if (close.includes(str[i]) && str[i] !== brackets[buffer.pop()]) {
      return false;
    }
  }
